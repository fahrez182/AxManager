...
open class AxeronService() : ServiceImpl() {
    ...
    companion object {
        ...
        const val TYPE_DEFAULT_ENV: Int = -1
        const val TYPE_ENV: Int = 0
        const val TYPE_NEW_ENV: Int = 1
        ...
        fun getDefaultEnvironment(): Environment {
            ...
            val built = Environment.Builder(false)
                .put("AXERON", "true")
                .put("AXERONDIR", PathHelper.getShellPath(ConstantEngine.folder.PARENT).absolutePath)
                .put("AXERONBIN", PathHelper.getShellPath(ConstantEngine.folder.PARENT_BINARY).absolutePath)
    ...
    private val axCompanion = File(PathHelper.getShellPath(ConstantEngine.folder.PARENT), "ax_perm_companion")
    ...
    @Volatile private var cachedMergedEnv: Environment? = null
    @Volatile private var cachedNewEnv: Environment? = null
    ...
    override fun setNewEnvironment(env: Environment) {
        ...
        cachedNewEnv = null
        shizukuUserServiceManager.environment = getEnvironment(TYPE_ENV)?.getEnv()
    }
    ...
    override fun getInfo(): AxeronInfo? {
        return AxeronInfo(
            VERSION_NAME,
            VERSION_CODE,
            PATCH_CODE,
